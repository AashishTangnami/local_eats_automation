...
def merchant_info(driver):
    ...
    # merchant_page.automate_information(ep.XPATH, 
    #                                    ep.MERCHANT_INFORMATION_XPATH, 
    #                                    ep.XPATH, ep.TAB_CONTENT, 
    #                                    TEXT_VALUE=None)
...
def settings(driver):
    ...
    # settings.order_options(ep.ID, ep.ORDER_VERIFICATION_NOTIFICATION_ID)
    # settings.order_options(ep.ID, ep.OTP_CODE_WAIT_TIME_ID)
    # settings.tax_options(ep.ID, ep.MERCHANT_TAX_ID, ep.MERCHANT_APPLY_TAX_ID, ep.MERCHANT_TAX)
    # settings.select_time_zone(ep.ID, ep.MERCHANT_TIMEZONE_ID)
    # settings.select_accept_preorders(ep.ID, ep.MERCHANT_PREORDER_ID)
    # settings.save(ep.XPATH, ep.SAVE_BUTTON)
    print("Settings Successful")

def food_category(driver):
    print('Food Category')
    food_category_obj = FoodCategory(driver)
    food_category_obj.navigate_to_food_category(ep.XPATH, ep.FOOD_CATEGORY)
...
def addon_category(driver):
    ...
    category_addon.navigate_to_addon_category(ep.XPATH, ep.FOOD_CATEGORY_NAV_XPATH)
